...
st.markdown(
    ...
    - The first model uses the  Top2Vec package which is relatively fast to train but has limited functionality
    - The second model s BERTopic which utilizes a natural language model (Bert) to generate topics and has additional high level visualisations built in. It takes sgnificantly longer to train on documents
    - Almost all outputs including displayed dataframes, tables and visualisations can be named and downloaded for later used
    - Both Top2Vec and BERTopic models can be downloaded and uploaded to for later use. They are downloaded as pickle files (.pkl)
    ...
    - Another is to load the newsapi dataframe or the Education Resources Information Center (ERIC) dataset which pulls data from Elastic Search and converts it to a json
