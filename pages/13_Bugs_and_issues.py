...
st.write('If a model is uploaded to the complex model, the dataframe must still be calcualted which takes time. This could be fixed by also suploading a previously saved dataframe however this adds complexity to the model memory flow as was too difficult to implement in the time given.')
st.write('There is not option to include more complex language embeddings which can improve results.  These were initally included but as they increased time taken for a model to train, they were removed and have not been added back in.')
