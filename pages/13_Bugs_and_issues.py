...
st.write('If a model is uploaded to the BERTopic model, the dataframe must still be calculated which takes time. This could be fixed by also suploading a previously saved dataframe however this adds complexity to the model memory flow as was too difficult to implement in the time given.')
st.write('There is not an option to include more BERTopic language embeddings which can improve results.  These were initally included but as they increased time taken for a model to train, they were removed and have not been added back in.')
...
st.write("There is currently no implementation of fuzzy matching of keywords. This would convert similar words to a single word such as nurse, nurses, nursing staff, nursing workers, nursing all to a single word. THis differs from lemmatization as each term is correct on its own. Implementing this would increase the power of the insights as less frequent terms wouldn't be overwhelmed by variations of the same word.")
