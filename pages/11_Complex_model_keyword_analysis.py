...
st.write('The Basic keyword analysis and the complex model pages must be run before proceeding.')
...
if "df_working" not in st.session_state:
    ...
    if 'keywords_df' not in st.session_state:
        ...
        final_complex_df = st.session_state.berttopic_model_df

        # sort dataframes by Title column
        final_complex_df = final_complex_df.sort_values(by=['Title'])
        # reset index
        final_complex_df = final_complex_df.reset_index(drop=True)
        # concat dfs
        final_complex_df = final_complex_df[['bert_topic_id']]#.drop(columns=['Title','bert_probability'],axis =1 )
        ...
        final_df_t2v = pd.concat([df_working,final_complex_df],axis = 1)

        st.subheader(' Complex model keyword analysis')
        ...
        st.download_button(
            ...
            file_name='Complex_topic_'+str(topic_slider_val)+'_most_common_words_per_year.csv',
            mime='text/csv'
        )
