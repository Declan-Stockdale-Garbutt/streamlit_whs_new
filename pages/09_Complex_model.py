...
st.write('A complex model is generated and trained on the uploaded json file. It takes considerably longer to train however it offers a lot more funcitonality')
st.write('Various dataframes are produced including a table of the ten msot representative documents for each topic along with a range of interactive visualization, all of which can be downlaoded as html files for later use.')
...
if 'df_working' not in st.session_state:
    st.subheader('Please upload a json file in previous page before proceeding with complex model')
else:
    ...
    if 'complex_model' not in st.session_state:# and 'berttopic_model_df' not in st.session_state:
        ...
        if uploaded_complex_model is not None:
            ...
            if st.session_state.berttopic_model_df is not None:
                del st.session_state.berttopic_model_df
        ...
        if st.session_state.complex_model is not None:
            ...
            if 'berttopic_model_df' not in st.session_state:
                ...
                if st.session_state.was_complex_model_uploaded == False:
