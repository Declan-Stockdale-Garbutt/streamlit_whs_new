...
show_more_useful_document_vis_button = st.button('Click to know more about the model', key = 'show_more_useful_document_vis ')

if show_more_useful_document_vis_button:
    st.write('The model used is the Top2Vec model. It relies on the assumption that similar documents are indicative of an underlying topic. The default embeddings are the Doc2Vec embeddings. Advanced embeddings can be implemented through modifcaiton of the code. More information on the package is available here https://github.com/ddangelov/Top2Vec')
...
if 'df_working' not in st.session_state:
    st.subheader('Please upload a json file in previous page')
else:
    ...
    if 'top2vec_model' not in st.session_state or st.session_state.top2vec_model == False:
        ...
        if 'top2vec_model_upload' not in st.session_state:
            st.session_state.top2vec_model_upload = False


        if 'proceed_button_t2v' not in st.session_state:
            st.session_state.proceed_button_t2v = False

        proceed_button_t2v = st.button('Generate Top2Vec model')
        ...
        st.write("")
        st.subheader('Upload saved model here')
        uploaded_top2vec_model = st.file_uploader("",key = 'uploaded_top2vec_model')

        if 'top2vec_model_uploaded' not in st.session_state:
            st.session_state.top2vec_model_uploaded = False

        if uploaded_top2vec_model is not None:

            top2vec_model = Top2Vec.load(uploaded_top2vec_model)
            st.session_state.top2vec_model = top2vec_model
            st.session_state.top2vec_model_uploaded = True
    ...
    #pass
        ...
        if remove_model:
            ...
            del st.session_state.final_t2v_df
            st.experimental_rerun()
        ...
        if st.session_state.top2vec_model_uploaded == False:
            ...
            download_model_top2vec = st.text_input('What would you like to name this the model', key = 'download_model_top2vec')
            if download_model_top2vec == "":
                download_model_top2vec = 'output'
            ...
            st.download_button(
                ...
                data=output_model,
                file_name=download_model_top2vec+ '.pkl'#,
            )
