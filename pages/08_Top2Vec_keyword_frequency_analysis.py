...
st.write('Run Basisc keyword analysis and Top2Vec model before loading this page.')
st.write('The Top2Vec model generates topic ids for each document while the keyword extactor generates keywords')
...
if "df_working" not in st.session_state:
    ...
    if 'keywords_df' not in st.session_state:
        ...
        st.write('Please run Top2Vec model to assign topics for each document then return to this page')
    else:
        ...
        # merge keywords, Top2Vec model topics for ocs and original df.
        ...
        # ready to model
        st.subheader(' Top2Vec model keyword analysis')
        view_top2vec_model_button = st.button('View keywords on Top2Vec model',key = "view_top2vec_model_button")


        # click to view
        if view_top2vec_model_button:
            st.dataframe(final_df_t2v)

            # click to close
            close_view_df_year = st.button('Close view')
            if close_view_df_year:
                view_top2vec_model_button = False


        # adding slider
        st.subheader('Find occurences of keyBert keywords by topic id assigned by Top2Vec over time')
        ...
        st.download_button(
            ...
            file_name='top2vec_topic_'+str(topic_slider_val)+'_most_common_words_per_year.csv',
            mime='text/csv'
        )
